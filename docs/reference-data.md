...
Three buckets: `immunities` (named effects), `moveTagImmunities` (per‑move classes like `fissure`/`horndrill`/`sheercold`, each layering the type‑agnostic `ohko` block on top of its own type‑immunity), `hazardRemoval` (per‑hazard).
