...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--suite')  { args.suite  = argv[++i]; continue; }
    if (a === '--team')   { args.team   = argv[++i]; continue; }
    if (a === '--tests')  { args.tests  = argv[++i]; continue; }
...
function printSummary(report) {
  ...
  process.stderr.write(`Team:  ${report.team.map(m => m.species).join(', ')}\n\n`);
  ...
  for (const r of report.results) {
    ...
    if (!r.pass && r.coverage?.uncovered?.length) {
      process.stderr.write(`       uncovered: ${r.coverage.uncovered.join(', ')}\n`);
    }
  }
  ...
  process.stderr.write(`  Warns: ${w.passed}/${(w.passed||0)+(w.failed||0)} passed`);
  process.stderr.write(`  Info:  ${i.passed}/${(i.passed||0)+(i.failed||0)} passed\n`);
  process.stderr.write(`Overall: ${report.passed ? '\x1b[32mPASS\x1b[0m' : '\x1b[31mFAIL\x1b[0m'}\n\n`);
}
...
try {
  const testIds = args.tests ? args.tests.split(',').map(s => s.trim()) : null;
